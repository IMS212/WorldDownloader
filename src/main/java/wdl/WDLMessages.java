...
public class WDLMessages {
	...
	public static void chatMessage(@Nonnull IConfiguration config,
		...
		chatMessage(config, type, new TextComponent(message));
	}
	 ...
	 * @param args
	 *            The arguments to pass to the {@link TranslatableComponent}.
	...
	public static void chatMessageTranslated(@Nonnull IConfiguration config,
		...
		for (int i = 0; i < args.length; i++) {
			if (args[i] == null) {
				TextComponent text = new TextComponent("null");
				text.setStyle(text.getStyle().withHoverEvent(new HoverEvent(Action.SHOW_TEXT, new TextComponent("~~null~~"))));
				args[i] = text;
		...
		if (I18n.exists(translationKey)) {
			component = new TranslatableComponent(translationKey, args);
		} else {
			...
			for (int i = 0; i < args.length; i++) {
				if (args[i] instanceof Component) {
					component.append((Component) args[i]);
				} else {
	 ...
	 */
	public static void chatMessage(@Nonnull IConfiguration config,
			@Nonnull IWDLMessageType type, @Nonnull Component message) {
		boolean enabled;
		try {
		...
		String tooltipText = I18n.get("wdl.messages.tooltip",
				type.getDisplayName().getString()).replace("\\n", "\n"); // XXX should be formatted
		Component tooltip = new TextComponent(tooltipText);

		BaseComponent text = new TextComponent("");

		TextComponent header = new TextComponent("[WorldDL]");
		header.setStyle(header.getStyle().withColor(TextColor.fromLegacyFormat(type.getTitleColor()))
				.withHoverEvent(new HoverEvent(Action.SHOW_TEXT, tooltip)));

		// If the message has its own style, it'll use that instead.
		// TODO: Better way?
		TextComponent messageFormat = new TextComponent(" ");
		messageFormat.setStyle(messageFormat.getStyle().withColor(TextColor.fromLegacyFormat(type.getTextColor())));
	...
	@Nonnull
	private static Component convertEntityToComponent(@Nonnull Entity e) {
		Component wdlName, displayName;

		try {
			String identifier = EntityUtils.getEntityType(e);
			if (identifier == null) {
				wdlName = new TranslatableComponent("wdl.messages.entityData.noKnownName");
			} else {
				...
				wdlName = new TextComponent(displayIdentifier);

				BaseComponent hoverText = new TextComponent("");
				hoverText.append(new TranslatableComponent("wdl.messages.entityData.internalName", identifier));
				hoverText.append("\n");
				hoverText.append(new TranslatableComponent("wdl.messages.entityData.group", displayGroup, group));

				((BaseComponent)wdlName).setStyle(
		...
		return new TranslatableComponent("wdl.messages.entityData", wdlName, displayName);
	}

	@Nonnull
	private static Component convertThrowableToComponent(@Nonnull Throwable t) {
		TextComponent component = new TextComponent(t.toString());
		...
		HoverEvent event = new HoverEvent(Action.SHOW_TEXT,
				new TextComponent(exceptionAsString));
