...
public class WDLMessages {
		...
		if (I18n.hasKey(translationKey)) {
			...
			for (int i = 0; i < args.length; i++) {
				if (args[i] instanceof ITextComponent) {
					component.append((ITextComponent) args[i]);
				} else {
					component.appendString(String.valueOf(args[i]));
				}
				if (i != args.length - 1) {
					component.appendString(", ");
				}
	...
	public static void chatMessage(@Nonnull IConfiguration config,
		...
		header.setStyle(header.getStyle().setColor(Color.fromTextFormatting(type.getTitleColor()))
		...
		messageFormat.setStyle(messageFormat.getStyle().setColor(Color.fromTextFormatting(type.getTextColor())));
	...
	private static ITextComponent convertEntityToComponent(@Nonnull Entity e) {
		...
		try {
			...
			if (identifier == null) {
				...
				hoverText.append(new TranslationTextComponent("wdl.messages.entityData.internalName", identifier));
				hoverText.appendString("\n");
				hoverText.append(new TranslationTextComponent("wdl.messages.entityData.group", displayGroup, group));
